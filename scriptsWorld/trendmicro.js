...
(async () => {
  ...
  await cluster.task(async ({ page, data: url }) => {
    if (first) {
      ...
      await page.goto(url);
      await attendre(5000, 5000);
      const selectPays = await page.$('body > main > div.reseller.vitl.section.full-browser-width-wrap > div.child-of-full-width > div > div.form-wrap.row > div:nth-child(1) > div > select');
      const payActuel = (pays + 1).toString();
      await selectPays.select(payActuel);
      const pays = await page.evaluate(select => {
        return select.options[select.selectedIndex].text;
      }, selectPays);
      const actualise = await page.$('body > main > div.reseller.vitl.section.full-browser-width-wrap > div.child-of-full-width > div > div.form-wrap.row > div.col-xs-12.col-sm-2.button-wrap > input');
      ...
      while (next) {
        ...
        for (i = 1; i <= donne.length; i++) {
          ...
          tel = '';
          tab = [nom, adresse, pays, tel, lien, 'TrendMicro'];
          csvData.push(tab);
        }
  ...
  first = false
  for (pays = 0; pays < totalPays.length; pays++) {
    await cluster.queue('https://www.trendmicro.com/en_us/partners/find-a-partner.html')
    await cluster.idle()
  }
  await cluster.close();
